...
export class RegimenComponent implements OnInit {
  ...
  trackId(index: number, item: IRegimen): number {
    return item.id!;
  }

  delete(regimen: IRegimen): void {
    const modalRef = this.modalService.open(RegimenDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.regimen = regimen;
  ...
  protected onSuccess(data: IRegimen[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/regimen'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
