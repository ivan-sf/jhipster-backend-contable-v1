...
export class TipoFacturaComponent implements OnInit {
  ...
  trackId(index: number, item: ITipoFactura): number {
    return item.id!;
  }

  delete(tipoFactura: ITipoFactura): void {
    const modalRef = this.modalService.open(TipoFacturaDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tipoFactura = tipoFactura;
  ...
  protected onSuccess(data: ITipoFactura[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
