...
export class ObjetoComponent implements OnInit {
  ...
  trackId(index: number, item: IObjeto): number {
    return item.id!;
  }

  delete(objeto: IObjeto): void {
    const modalRef = this.modalService.open(ObjetoDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IObjeto[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
