...
export class CodigoComponent implements OnInit {
  ...
  trackId(index: number, item: ICodigo): number {
    return item.id!;
  }

  delete(codigo: ICodigo): void {
    const modalRef = this.modalService.open(CodigoDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ICodigo[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
