...
export class EmpresaService {
  ...
  protected convertDateFromClient(empresa: IEmpresa): IEmpresa {
    return Object.assign({}, empresa, {
      fechaRegistro: empresa.fechaRegistro?.isValid() ? empresa.fechaRegistro.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((empresa: IEmpresa) => {
        empresa.fechaRegistro = empresa.fechaRegistro ? dayjs(empresa.fechaRegistro) : undefined;
      });
    }
    return res;
  }
