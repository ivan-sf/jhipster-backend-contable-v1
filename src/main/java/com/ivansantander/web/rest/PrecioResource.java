...
/**
 * REST controller for managing {@link com.ivansantander.domain.Precio}.
 */
@RestController
@RequestMapping("/api")
public class PrecioResource {
    ...
    public ResponseEntity<Precio> updatePrecio(@PathVariable(value = "id", required = false) final Long id, @RequestBody Precio precio)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, precio.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Precio> partialUpdatePrecio(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, precio.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<Precio>> getAllPrecios(Pageable pageable) {
        ...
        Page<Precio> page = precioService.findAll(pageable);
